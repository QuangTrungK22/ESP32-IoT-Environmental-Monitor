...
void task_webserver(void *pvParameters) {
    // Setup ban đầu (nếu cần)
    Serial.println("Webserver Task Started");
    
    while(1) {
        // Gọi hàm reconnect (đã có sẵn logic kiểm tra và khởi động lại server)
        Webserver_reconnect();
        
        // Quan trọng: Delay nhẹ để nhường CPU cho task khác, tránh treo chip
